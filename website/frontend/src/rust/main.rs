...
use image::*;
use speedy2d::{color::*, dimen::Vector2, font::*, image::*, shape::*, GLRenderer};
...
//dyn_clone crate used for convenient cloning of heap allocated trait objects
// (Box<dyn CanvasitemExt>)
...
impl CanvasImage {
    pub fn new(path: &str, pos: Vector2<f32>, scale: f32) -> Self {
        ...
        let img_type = match has_alpha {
            true => ImageDataType::RGBA,
            false => ImageDataType::RGB,
        };

        let (ci_width, ci_height) = ImageReader::open(path)
            .unwrap()
            .decode()
            .unwrap()
            .dimensions();
...
impl std::fmt::Debug for CanvasImage {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "Canvas Image: data len {}, width: {}, height: {}, pos: {}, {}",
            self.data.len(),
            self.width,
            self.height,
            self.pos.x,
...
impl CanvasItemExt for CanvasImage {
    ...
    fn draw(
        ...
        let rect_pos_2 = world_to_screen_2(
            Vector2::new(coords.x + img_width as f32, coords.y + img_height as f32),
            c_offset,
            c_scale,
        );
...
impl std::fmt::Debug for CanvasCircle {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "Canvas Circle: radius: {}, pos: {}, {}",
            self.radius, self.pos.x, self.pos.y
        )
    }
}
...
fn main() {
    ...
    let mut renderer =
        unsafe { GLRenderer::new_for_current_context((wind_size as u32, wind_size as u32)) }
            .unwrap();

    let container = CanvasContainer::new(CanvasState {
        offset: Vector2::new(0.0, 0.0),
        items: vec![
            Box::new(CanvasCircle::new(
                Vector2::new(1200.0, 200.0),
                100.0,
                Color::RED,
            )),
            Box::new(CanvasCircle::new(
                Vector2::new(1300.0, 200.0),
                100.0,
                Color::GREEN,
            )),
            Box::new(CanvasCircle::new(
                Vector2::new(1400.0, 200.0),
                100.0,
                Color::BLUE,
            )),
        ],
    ...
    win.handle({
        ...
        move |_, ev| match ev {
            ...
            Event::MouseWheel => {
                match app::event_dy() {
                    app::MouseWheel::Down => {
                        container_rc_handle.zoom(
                            app::event_x_root() - main_win.x(),
                            app::event_y_root() - main_win.y(),
                            1.05,
                        );
                    }
                    app::MouseWheel::Up => {
    ...
    win.draw({
        ...
        let bottom_bar_rect = Rectangle::new(
            Vector2::new(0.0, wind_size as f32 - 20.0),
            Vector2::new(wind_size as f32, wind_size as f32),
        );
        move |_| {
            renderer.draw_frame(|graphics| {
                ...
                let zoom_text = helvetica_font.layout_text(
                    &format!(
                        ...
                        format!("{:.1$}", scene.zoom_scale.y.to_string(), 4)
                    ),
                    font_size,
                    TextOptions::new(),
                );
                ...
                let offset_text = helvetica_font.layout_text(
                    &format!(
                        ...
                        format!("{:.1$}", scene.offset.y.to_string(), 5)
                    ),
                    font_size,
                    TextOptions::new(),
                );
                graphics
                    .draw_rectangle(bottom_bar_rect.clone(), Color::from_int_rgba(0, 0, 0, 150));
                graphics.draw_text((0.0, wind_size as f32 - text_y), Color::WHITE, &zoom_text);
                graphics.draw_text(
                    (180.0, wind_size as f32 - text_y),
                    Color::WHITE,
                    &event_text,
                );
                graphics.draw_text(
                    (360.0, wind_size as f32 - text_y),
                    Color::WHITE,
                    &offset_text,
                );
            });
